...
import csv
import os
from sensor_validation import sensor_validation_errors

# Configuration — Arduino only, no simulation
ARDUINO_PORT = 'COM8'  # Default port; overridden by user via Engine Controls
...
_MODEL_PATHS = {
    # Canonical artifacts are the selected best models. Named artifacts below
    # must remain tied to the algorithm named in their filename.
...
def load_models():
    ...
    global dt_current_model, gb_current_model, knn_current_model, rf_current_model
    global dt_forecast_model, gb_forecast_model, knn_forecast_model, rf_forecast_model
    ...
    with _models_lock:
        ...
        rf_current_model   = _try_load(_resolve('random_forest_current.pkl'))
        ...
        rf_forecast_model  = _try_load(_resolve('random_forest_forecast.pkl'))
...
def log_data(data):
    """Log only sensor records with an actual, trainable weather label.

    Operational states are dashboard statuses, not weather observations, and
    must never be written where dataset generation could use them as labels.
    """
    if data.get("current_weather") not in TRAINABLE_WEATHER_LABELS:
        print("[LOG] Skipped record without a trustworthy weather label.")
        return False
                            ...
                            if len(values) == 5:
                                try:
                                    ...
                                    if not validate_sensor_input(t, h, p, uv, pres):
                                        # Publish the rejected-packet status without touching smoothing.
                                        with data_lock:
...
def validate_sensor_input(t, h, p, uv, pres):
    ...
    """
    warnings = sensor_validation_errors(t, h, p, uv, pres)
    if warnings:
        print(f"[REJECTED] Invalid sensor reading: {', '.join(warnings)}")
        return False
    return True

    # Legacy checks retained below only for source-history context; they are
    # unreachable because shared validation above is authoritative.
    warnings = []
...
def update_values(t, h, p, uv, pres):
    ...
    # Mandatory prediction boundary: invalid packets never enter smoothing,
    # scaling, prediction, or voting.
    if not validate_sensor_input(t, h, p, uv, pres):
        sensor_data["status"] = "Invalid sensor reading"
        sensor_data["arduino_warning"] = "Invalid sensor reading rejected; waiting for the next valid packet."
        sensor_data["prediction"] = None
        sensor_data["forecast_2hr"] = None
        sensor_data["confidence"] = None
        return False
    ...
    if scaler and label_encoder:
        try:
            ...
            _c_ensemble = [
                ...
                ("random_forest",     rf_current_model,  model_accuracies.get("random_forest",     {}).get("current_accuracy", 0.99)),
            ]
    ...
    if forecast_scaler and forecast_encoder:
        try:
            ...
            _f_ensemble = [
                ...
                ("random_forest",     rf_forecast_model,  model_accuracies.get("random_forest",     {}).get("forecast_accuracy", 0.60)),
            ]
    ...
    if scaler and label_encoder and forecast_encoder:
        try:
            ...
            _multi = [
                ...
                ("random_forest",     rf_current_model,  rf_forecast_model),
            ]
...
def _run_simulation():
    ...
    while True:
        ...
        with data_lock:
            update_values(t, h, p, uv, pres)
            # Simulation is display-only. Generated readings are never
            # persisted as data that could later become training examples.
