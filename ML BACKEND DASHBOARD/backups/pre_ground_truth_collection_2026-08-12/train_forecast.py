...
def train_models():
    try:
        ...
        observed_labels = set(df['Condition'].unique()) | set(df['Target_Condition'].unique())
        operational_labels = observed_labels - TRAINABLE_WEATHER_LABELS
        if operational_labels:
            raise ValueError(
                "Refusing to train on non-weather operational labels: "
                f"{sorted(operational_labels)}"
            )
        if df['Condition'].nunique() < 2 or df['Target_Condition'].nunique() < 2:
            raise ValueError(
                "Refusing to train: current and forecast targets each need at least two trustworthy weather classes"
            )
        print(f"\nDataset: {len(df)} rows\n")
