...
def validate_sensor_input(t, h, p, uv, pres):
    """Reject invalid packets before they enter smoothing or ML inference."""
    errors = sensor_validation_errors(t, h, p, uv, pres)
    if errors:
        print(f"   [REJECTED] Invalid sensor reading: {', '.join(errors)}")
        return False
    return True
        ...
        try:
            ...
            if line:
                ...
                if len(values) == 5:
                    ...
                    # Reject before history, scaling, prediction, and voting.
                    # Invalid packets therefore cannot affect a later moving average.
